<template>
  <div class="card card-flush shadow-sm h-100 border-0 mobile-revenue-card">
    <div class="card-header pt-7 border-0">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bold text-gray-800 fs-5">Tren Pendapatan</span>
        <span class="text-muted mt-1 fw-semibold fs-8">Statistik pendapatan harian</span>
      </h3>
    </div>
...
const renderChart = () => {
  const element = document.getElementById('mobile_revenue_area_chart')
  ...
  const options = {
    series: [{ name: 'Pendapatan', data: safeData.map(d => d.y) }],
    chart: {
      type: 'area',
      height: 260,
      width: '100%',
    ...
    yaxis: {
      labels: {
        ...
        style: { fontSize: '10px', colors: '#A1A5B7' }
      }
    },
